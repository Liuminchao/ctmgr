...
if (is_array($rows)) {
    ...
    foreach ($rows as $i => $row) {
        ...
        $t->echo_td($row['root_proname'],'center'); //总包名称
...
//        $t->echo_td($row['Violation_record']); //违规记录
        ...
        if (Yii::app()->language == 'zh_CN') {
            $t->echo_td($row['assess_name'],'center');
        }else{
            $t->echo_td($row['assess_name_en'],'center');
        }
        //事件选项
        if (Yii::app()->language == 'zh_CN') {
            $t->echo_td($row['item_name'],'center');
        }else{
            $t->echo_td($row['item_name_en'],'center');
        }
        $t->echo_td(Utils::DateToEn($row['apply_time']),'center');//申请时间
        $t->echo_td(Utils::WorkDateToEn($row['deadline_date']),'center'); //截止时间
