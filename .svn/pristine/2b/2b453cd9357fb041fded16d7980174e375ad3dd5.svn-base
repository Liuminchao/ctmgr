...
class UserModuleCommand extends CConsoleCommand
{
    ...
    //0,10 3-4 * * * php /opt/www-nginx/web/test/ctmgr/protected/yiic usermodule bach
...
//        exit;
        ...
        $filename = "/opt/www-nginx/web/filebase/tmp/20210405_1300.zip";
        $user_id = '1300';
//        unlink($filename);
//
...
//        echo 'PTW';
//        $sql = "SELECT apply_id FROM `ptw_apply_worker` where user_id = '1300'";
...
////            var_dump($params['id']);
//                $app_id = 'PTW';
//                  $add_operator = explode('|',$ptw_apply->add_operator);
//                  $step = $add_operator[0];
//                  $year = substr($ptw_apply->record_time,0,4);//年
//                $deal_type = CheckApplyDetail::dealtypeList($app_id, $j['apply_id'], $step);
//                echo $j['apply_id'].' ';
//                $filepath = $ptw_apply->save_path;
//                if(!$filepath){
...
//        $command = Yii::app()->db->createCommand($sql);
//        $rows = $command->queryAll();
////        $rows[0]['meeting_id'] = '1575590140334';
//        foreach($rows as $i => $j){
