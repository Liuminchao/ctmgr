...
//        $t->begin_row();
        ...
        if ($row['status'] == PayrollSalarysummary::STATUS_NORMAL) { //状态是未入库的 可以点击查看详细工资
            
            $link = "<a href='javascript:void(0)' onclick='itemDetail(\"{$row['summary_id']}\",\"{$row['start_date']}\")'><i class=\"fa fa-fw fa-bell\"></i>".Yii::t('pay_payroll', 'detail_wage')."</a>";    //详细工资
        }
        else {
//            $link = "<a ><i class=\"fa fa-fw fa-edit\"></i>".Yii::t('common', 'edit')."</a>";
            $link = '';
        }
//        $t->echo_td(Utils::MonthToEn($row['wage_month']));
        $t->echo_td(Utils::WorkDateToEn($row['start_date']));
        $t->echo_td(Utils::WorkDateToEn($row['end_date']));
    ...
    <div class="col-xs-3">
        <div class="dataTables_info" id="example2_info">
            ...
            <?php if($rows){ ?>
                ...
                <a class="right" style="cursor: pointer;"  id="export"><strong onclick="itemExport();"><?php echo Yii::t('pay_payroll', 'Salary_export');?></strong></a>
            <?php } ?>
        </div>
    </div>
    <div class="col-xs-9">
