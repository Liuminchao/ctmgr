...
class ProjectController extends AuthBaseController {
    ...
    private function genDataGrid() {
        ...
        if($ptype=='MC'){
            ...
            $t->set_header(Yii::t('proj_project','project_region'),'80%','center');
        }else{
    ...
    public function actionGrid() {
        ...
        $this->renderPartial('_list', array('t' => $t,'program_id'=>$args['program_id'],'rows' => $list['rows'], 'new_cnt' => $list['total_num'], 'curpage' => $list['page_num']));
    }
    ...
    public function actionAppList() {
        ...
        //默认勾选模块数组
        $app = '';
        $select_list = ProgramApp::myAppList($program_id,$app);
        //模块列表
        $app = '2';
    ...
    public function actionTabs() {
        ...
        if($mode == 'insert'){
            ...
            if($title == '1'){
                ...
                $this->smallHeader = Yii::t('proj_project', 'Base Info');
            }
            ...
            //$model->construction_start = Utils::DateToEn($model->construction_start);
            //$model->construction_end = Utils::DateToEn($model->construction_end);
