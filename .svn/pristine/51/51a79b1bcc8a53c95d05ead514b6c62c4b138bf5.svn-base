 ...
 * @author LiuMinchao
 */
class TrainingController extends AuthBaseController {
    ...
    public function init() {
        parent::init();
        $this->contentHeader = Yii::t('train', 'contentHeader');
        $this->bigMenu = Yii::t('train', 'bigMenu');
    }
    ...
    private function genDataGrid() {
        ...
        $t->url = 'index.php?r=train/training/grid';
        $t->updateDom = 'datagrid';
        $t->set_header(Yii::t('train', 'seq'), '', 'center');
        $t->set_header(Yii::t('train', 'program_name'), '', 'center');
        $t->set_header(Yii::t('train', 'title'), '', 'center');
        $t->set_header(Yii::t('train','training_type'),'','center');
        $t->set_header(Yii::t('train', 'apply_date'), '', 'center');
        $t->set_header(Yii::t('train', 'status'), '', 'center');
    ...
    public function actionList() {
        ...
        $this->smallHeader = Yii::t('train', 'smallHeader List');
...
//        Yii::import('application.extensions.tcpdf.TCPDF');
        ...
        // 设置页眉和页脚信息
        $pdf->SetHeaderData('', 0, 'Training/Meeting No.(培训/会议编号): ' . $meeting->training_id, $title, array(0, 64, 255), array(0, 64, 128));
...
//        $end_date = date('Y-m-d',strtotime($meeting->end_date));
        ...
        //培训详情
        $work_content_html = "<h4 align=\"center\">Training Details (培训详情)</h4><table border=\"1\">";
        $work_content_html .="<tr><td width=\"33%\" nowrap=\"nowrap\" bgcolor=\"#d9d9d9\" align=\"center\">Title (标题)</td><td width=\"34%\" nowrap=\"nowrap\" bgcolor=\"#d9d9d9\" align=\"center\">Content (内容)</td><td width=\"33%\" nowrap=\"nowrap\" bgcolor=\"#d9d9d9\" align=\"center\">Training Type (培训类型)</td></tr>";
        ...
        $work_date_html .="<tr><td width=\"10%\" nowrap=\"nowrap\" bgcolor=\"#d9d9d9\" align=\"center\">S/N (序号)</td><td width=\"30%\" nowrap=\"nowrap\" bgcolor=\"#d9d9d9\" align=\"center\">Date of Training (培训日期)</td><td width=\"30%\" nowrap=\"nowrap\" bgcolor=\"#d9d9d9\" align=\"center\">Start Time (开始时间)</td><td width=\"30%\" nowrap=\"nowrap\" bgcolor=\"#d9d9d9\" align=\"center\">End Time (结束时间)</td></tr>";
...
//                $pic = 'C:\Users\minchao\Desktop\5.png';
        ...
        if (!empty($progress_list))
            ...
            if($num < 5) {
                ...
                $html = $title_html . $apply_info_html . $work_content_html . $work_date_html . $pic_html  .$audit_html . $worker_html;
