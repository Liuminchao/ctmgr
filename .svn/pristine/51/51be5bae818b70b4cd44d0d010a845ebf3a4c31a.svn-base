...
if (is_array($rows)) {
    ...
    foreach ($rows as $i => $row) {
        ...
        $pro_model = Program::model()->findAllByPk($row['root_proid']);
        $t->echo_td($pro_model[0]['program_name']); //总包名称
        ...
        $preview_link = "<a href='javascript:void(0)' onclick='itemPreview(\"{$row['check_id']}\")'><i class=\"fa fa-fw fa-file-text-o\"></i>Preview</a>";
        $workflow_link = "<a href='javascript:void(0)' onclick='itemWorkflow(\"{$row['check_id']}\")'><i class=\"fa fa-fw fa-bar-chart-o\"></i>" . Yii::t('sys_workflow', 'Approval Process') . "</a>";
...
//        $staff_link ="<a href='javascript:void(0)' onclick='itemStaff(\"{$row['apply_id']}\",\"{$app_id}\")'><i class=\"fa fa-fw fa-users\"></i>" . Yii::t('sys_workflow', 'construction personnel') . "</a>";
        ...
        if($row['status'] == 1 || $row['status'] == 2){    //完成后
            $link .= "<table><tr><td style='white-space: nowrap'>$download_link</td><td style='white-space: nowrap'>$preview_link</td></tr></table>";
        }
        else{
