...
class ProjectAttend extends CActiveRecord {
	...
	public static function queryProgramAttend($month,$program_id,$type_id,$contractor_id){
        ...
        $table_prefix_1 = 'atd_records';
        $table_name_1 = $table_prefix_1.'_'.$month;

//        var_dump($rows_1);
//        exit;
        $sql_2 = "select * from bac_program_user where program_id ='".$program_id."' and contractor_id='".$contractor_id."' ";
        ...
        if($hours == 0){
            ...
            $sql_1 = "select user_id,SUBSTRING(card_time,1,10) as card_time from $table_name_1 where device_id ='".$program_id."' and org_id='".$contractor_id."' ";
            $command_1 = Yii::app()->db->createCommand($sql_1);
            $rows_1 = $command_1->queryAll();
            $re =array();
            foreach($rows_1 as $m => $n){
                if(is_array($re[$n['card_time']])){
                    if(!in_array($n['user_id'],$re[$n['card_time']])){
                        $re[$n['card_time']][] = $n['user_id'];
                    }
                }else{
                    $re[$n['card_time']][] = $n['user_id'];
                }
        ...
        if($hours == 0){
            ...
            if(count($list)>0){
                foreach($list as $x => $user_list){
                    $user_array = array();
                    $rs[$x]['act_hour'] = 0;
                    foreach($re as $date =>$fix_list){
                        foreach($fix_list as $i => $user_id){
