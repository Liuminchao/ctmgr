...
class DeviceType extends CActiveRecord {
    ...
    public static function deviceList(){
        ...
        $sql = "SELECT device_type_ch,device_type_en,type_no FROM bac_device_type WHERE status=00 ";//var_dump($sql);
        $command = Yii::app()->db->createCommand($sql);
        $rows = $command->queryAll();
        if (count($rows) > 0) {
            if (Yii::app()->language == 'zh_CN') {
                foreach ($rows as $key => $row) {
                    $rs[$row['type_no']] = $row['device_type_ch'];
                }
            } else if (Yii::app()->language == 'en_US') {
                foreach ($rows as $key => $row) {
                    $rs[$row['type_no']] = $row['device_type_en'];
                }
            }else{
                foreach ($rows as $key => $row) {
                    $rs[$row['type_no']] = $row['device_type_en'];
