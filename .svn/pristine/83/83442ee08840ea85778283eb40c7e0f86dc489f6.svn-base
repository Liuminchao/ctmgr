...
<?php echo $this->gridId; ?>.refresh();
    ...
    var itemAdd = function () {
        window.location = "index.php?r=sys/operator/new";
    }
    //修改
    var itemEdit = function (id) {
        window.location = "index.php?r=sys/operator/edit&id=" + id;
    }
    //注销
    var itemLogout = function (id, name) {

        if (!confirm('<?php echo Yii::t('sys_operator', 'Confirm Logout_1'); ?>' + name + '<?php echo Yii::t('sys_operator', 'Confirm Logout_2'); ?>')) {
            return;
        }
        $.ajax({
            data: {id: id, confirm: 1},
            url: "index.php?r=sys/operator/logout",
            dataType: "json",
            type: "POST",
    ...
    var itemResetPwd = function (id, name) {
        ...
        $.ajax({
            data: {id: id, confirm: 1},
            url: "index.php?r=sys/operator/resetpwd",
            dataType: "json",
            type: "POST",
            success: function (data) {
                alert(data.msg);
    ...
    var getDetail = function (obj, id) {
        ...
        $.ajax({
            data: {id: id},
            url: "index.php?r=sys/operator/detail",
            type: "POST",
            dataType: "json",
            beforeSend: function () {
