...
//                $month = Utils::DateToEn(date('m',strtotime('+1 day')));
                    ...
                    <div class="input-group has-error">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control input-sm tool-a-search" name="q[month]" 
                                   value="<?php echo $month; ?>" id="q_start_date" onclick="WdatePicker({lang:'en',dateFmt:'MMM yyyy'})" placeholder="月份"/>
                        </div>
                    </div>
                </div>               
    ...
    <div class="col-xs-3">
        <div class="dataTables_filter" >
            <label>          
                <button class="btn btn-primary btn-sm" onclick="itemSet()"><?php echo Yii::t('pay_payroll', 'button_set');?></button>
            </label>
        </div>
    </div>
</div>
