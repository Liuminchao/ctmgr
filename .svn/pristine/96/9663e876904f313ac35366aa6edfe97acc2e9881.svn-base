...
class Staff extends CActiveRecord {
     ...
     * @return type
     */
    public static function getArrayNationality() {
        $rows = array(
                'Chinese'   =>  'China',
    ...
    /**
     * 得到国籍名称
     * @param type $nation_type 国家id
     * @return string
     */
    public static function getNationalityName($nation_type) {
        $nationality_list = self::getArrayNationality();
        if (empty($nation_type)) {
            return '';
        }
        return array_key_exists($nation_type, $nationality_list) ? $nationality_list[$nation_type] : $nation_type;
    }
    ...
    public static function queryListByProgram($page, $pageSize, $args = array()) {
        ...
        if($args['tag'] == 'out'){
            $criteria->join = "RIGHT JOIN bac_program_user b ON b.root_proid = '$root_proid' and t.user_id = b.user_id and b.check_status in (10)";
        }else{
            $criteria->join = "RIGHT JOIN bac_program_user b ON b.root_proid = '$root_proid' and t.user_id = b.user_id and b.check_status in (11,20)";
        }
        $criteria->params = $params;
        $criteria->order = $order;
        // $pages = new CPagination($total_num);
    ...
    public static function insertStaff($args,$infoargs) {
        ...
        if ($exist_data != 0) {
            $sql = "SELECT a.user_name,b.contractor_name FROM bac_staff a,bac_contractor b WHERE a.user_phone = :user_phone and a.contractor_id = b.contractor_id ";
        ...
        try{
            ...
            $model->user_name = addslashes(trim($args['user_name']));
    ...
    public static function updateStaff($args,$infoargs) {
       ...
       // 判断uer_id
        ...
        $user_phone_edit = str_replace(' ','',$args['user_phone']);
        $work_no_edit = str_replace(' ','',$args['work_no']);
    ...
    */
    public static function logoutStaff($id) {
        ...
        try {
            ...
            if ($result) {
                $del_status = self::STATUS_DISABLE;//注销
                ...
                $r['msg'] = $id.' '.Yii::t('common', 'success_logout');
                $r['status'] = 1;
                $r['refresh'] = true;
            } else {
                $r['msg'] = $id.' '.Yii::t('common', 'error _logout');
                $r['status'] = -1;
                $r['refresh'] = false;
            }
        } catch (Exception $e) {
    ...
    public static function userById($user_id) {
        $sql = "SELECT user_name FROM bac_staff WHERE user_id=:user_id and status=0";
        $command = Yii::app()->db->createCommand($sql);
