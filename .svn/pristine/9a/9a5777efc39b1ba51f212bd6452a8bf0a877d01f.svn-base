...
<?php echo $this->gridId; ?>.refresh();
    ...
    //查询设备证书
    var itemPhoto = function (primary_id) {
        window.location = "index.php?r=device/equipment/attachlist&primary_id="+primary_id;
    }
    //统计信息
        ...
        $("table :checkbox").each(function(key,value){
            if(key != 0) {
                if ($(value).prop('checked')) {
                    var primary_id = tbodyObj.rows[key].cells[1].innerHTML;
                    var device_name =tbodyObj.rows[key].cells[4].innerHTML;
                    id += primary_id + '|';
                    name +=device_name + ',';
                }
            }
