 ...
 * @author LiuMinChao
 */
class ConsassAudit extends CActiveRecord {
    ...
    public static function queryList($page, $pageSize, $args = array()) {
        ...
        $total_num = ConsassAudit::model()->count($condition, $params); //总记录数
        ...
        $rows = ConsassAudit::model()->findAll($criteria);
    ...
    public static function InsertList($data,$program_id){
        ...
        try {
            foreach ($data as $k => $v){
                $sub_sql = 'INSERT INTO bac_consass_audit(program_id,contractor_id,proposed_date,actual_date,audit_company,remarks,record_time,status) VALUES(:program_id,:contractor_id,:proposed_date,:actual_date,:audit_company,:remarks,:record_time,:status)';
                ...
                $command->bindParam(":proposed_date", $v->proposed_date, PDO::PARAM_STR);
                $command->bindParam(":actual_date", $v->actual_date, PDO::PARAM_STR);
                $command->bindParam(":audit_company", $v->audit_company, PDO::PARAM_STR);
    ...
    public static function UpdateList($id,$args){
        $status = self::STATUS_NORMAL;
        $model = ConsassAudit::model()->findByPk($id);
        $trans = Yii::app()->db->beginTransaction();
        try {
            $model->program_id = $args['program_id'];
            $model->proposed_date = $args['proposed_date'];
            $model->actual_date = $args['actual_date'];
            $model->audit_company = $args['audit_company'];
...
//        $sql = "delete from bac_consass_audit where id = '".$id."' ";
