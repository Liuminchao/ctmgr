 ...
 * @author LiuMinchao
 */
class SafetyCheckDetail extends CActiveRecord {
    ...
    public function tableName() {
        return 'bac_safety_check_detail';
    }
    ...
    public static function detailList($check_id){
        ...
        $sql = "SELECT * FROM bac_safety_check_detail WHERE  check_id = '".$check_id."' ";//var_dump($sql);
    ...
    //根据安全单编号查询安全单步骤deal_type = 3
    public static function detailFinishTimeList($check_id){

        $sql = "SELECT record_time FROM bac_safety_check_detail WHERE  deal_type = 3 and check_id = '".$check_id."' ";
        $command = Yii::app()->db->createCommand($sql);
        $rows = $command->queryall();

        return $rows;
    }
    //根据安全单编号查询安全单步骤deal_type = 2结束维修时间
    public static function detailRectifiedTimeList($check_id){

        $sql = "SELECT check_id ,record_time FROM bac_safety_check_detail WHERE deal_type = 2 and check_id = '".$check_id."' order by record_time desc";
        $command = Yii::app()->db->createCommand($sql);
        $rows = $command->queryall();
        return $rows;
    }
