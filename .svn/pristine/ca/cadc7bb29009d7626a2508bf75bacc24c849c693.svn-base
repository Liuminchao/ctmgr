...
//            $infomodel->ins_adt_expire_date = Utils::DateToCn($infoargs['ins_adt_expire_date']);
            $res = $infomodel->save();
            //压缩头像
            if(file_exists($infoargs['face_img'])){
                $img_array = explode('/',$infoargs['face_img']);
                $index = count($img_array) -1;
                $img_array[$index] = 'thumb_'.$img_array[$index];
                $thumb_img = implode('/',$img_array);
                $stat = Utils::img2thumb($infoargs['face_img'], $thumb_img, $width = 0, $height = 100, $cut = 0, $proportion = 0);
            }
...
//         exit;
            $infomodel->face_img = '/opt/www-nginx/web'.$infoargs['face_img'];  //图片路径
            $infoargs['face_img'] = '/opt/www-nginx/web'.$infoargs['face_img'];  //图片路径
        }
//        var_dump($infoargs);
//        exit;
...
//            $infomodel->ins_adt_expire_date = Utils::DateToCn($infoargs['ins_adt_expire_date']);
            ...
            if(!$res){
                ...
                //压缩头像
                $face_img = '/opt/www-nginx/web'.$infoargs['face_img'];
                if(file_exists($face_img)){
                    $img_array = explode('/',$face_img);
                    $index = count($img_array) -1;
                    $img_array[$index] = 'thumb_'.$img_array[$index];
                    $thumb_img = implode('/',$img_array);
                    $stat = Utils::img2thumb($face_img, $thumb_img, $width = 0, $height = 100, $cut = 0, $proportion = 0);
                }
