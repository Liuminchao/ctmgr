...
//            url: "index.php?r=ra/raswp/demodata",
            ...
            cols: [[
                ...
                {field: 'proposed_date', title: 'Proposed Audit Date', edit: 'text'},
                {field: 'actual_date', title: 'Actual Audit Date', edit: 'text'},
                {field: 'audit_company', title: 'Audit Company', edit: 'text'},
        ...
        var active = {
            addRow: function(){	//添加一行
                ...
                console.log(oldData);
                var newRow = {tempId: new Date().valueOf(), proposed_date: 'DD/MM/YYYY',actual_date: 'DD/MM/YYYY', audit_company: null, remarks: null};
                oldData.push(newRow);
                console.log(oldData);
                tableIns.reload({
                    data : oldData
            ...
            save: function(){
                ...
                for(var i=0, row; i < oldData.length; i++){
                    row = oldData[i];
                    if(!row.audit_company){
                        layer.msg("填写Audit Company！", { icon: 5 }); //提示
                        return;
                    }
                }
...
//                document.getElementById("jsonResult").innerHTML = JSON.stringify(table.cache[layTableId], null, 2);	//使用JSON.stringify() 格式化输出JSON字符串
                ...
                $.ajax({
                    ...
                    url: "index.php?r=device/equipment/addconsass",
                    type: "POST",
                    dataType: "json",
                    beforeSend: function () {
