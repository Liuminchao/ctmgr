...
class FtpReportCommand extends CConsoleCommand
{
    ...
    public function actionBach()
    {
        ...
        foreach($program_list as $x => $program_id){
            ...
            foreach($rows as $i => $j){
                ...
                if($j['status'] == 1 || $j['status'] == 2){
                    if(!file_exists($j['save_path']) && $j['contractor_id'] != ''){
                        $n++;
                        echo $j['check_id'];
                        $filepath = DownloadPdf::transferDownload($params,$app_id);
                        if(file_exists($filepath)){
    ...
    public function actionBachEpss()
    {
        $program_id = '212';
        $args['program_id'] = '212';
        $args['contractor_id'] = '146';
        $args['type_id'] = '1';
        $args['month'] =  Utils::MonthToEn(date('Y-m',strtotime('-1 month')));
        $month_cn = date('Y-m',strtotime('-1 month'));
        $args['filepath'] = '/opt/www-nginx/web/filebase/tmp/'.$month_cn.'.xls';
        $year = substr($month_cn,0,4);
        $month = substr($month_cn,5,2);
        $remote_file = 'SFTP/REPORT/'.$program_id.'/CHECKLIST/'.$year.'/'.$month;
        EpssRole::epssReportByFtp($args);
        $filepath_array = explode('/',$args['filepath']);
        $count = count($filepath_array);
        $file = $filepath_array[$count-1];
        Ftp::upload($args['filepath'],$remote_file,$file);
    }
}