...
if (is_array($rows)) {
    ...
    foreach ($rows as $i => $row) {
        ...
        $t->echo_td(Utils::DateToEn($row['record_time']));//考勤时间
        ...
        if($row['record_status'] != '00'){
            $t->echo_td("<a href='javascript:void(0);' class='img_class' img_url='".$row['img_url']."'><img width='30' src='index.php?r=sys/attend/viewImg&img_url=".$row['img_url']."'/></a>");
        }else{
            $t->echo_td('');
        }
        $t->end_row();
...
<script type="text/javascript">
    ...
    function setImg(img_url,obj){
        var src,h;
        src='index.php?r=sys/attend/viewImg&img_url='+img_url;//alert(src);
