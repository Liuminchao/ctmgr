 ...
 * @author LiuXiaoyuan
 */
class PtwCondition extends CActiveRecord
{
    ...
    public static function queryList($page, $pageSize, $args = array()) {
        ...
        $total_num = PtwCondition::model()->count($condition, $params); //总记录数
        ...
        $rows = PtwCondition::model()->findAll($criteria);
    ...
    //插入数据
    public static function insertPtwCondition($args,$data) {

        foreach ($args as $key => $value) {
            $args[$key] = trim($value);
        }

        $exist_data = PtwCondition::model()->count('condition_id=:condition_id', array('condition_id' => $args['condition_id']));
        if ($exist_data != 0) {
        ...
        if($r['status'] == 1){
            try {
                ...
                $sub_sql = 'INSERT INTO ptw_condition_list(type_id,condition_name,condition_name_en,status,record_time) VALUES(:type_id,:condition_name,:condition_name_en,:status,:record_time);';
                ...
                foreach($data as $k => $v) {
                    ...
                    $command->bindParam(":record_time",$record_time, PDO::PARAM_INT);
                    $rs = $command->execute();
                }
    ...
    //添加数据
    public static function setPtwCondition($args) {
//        var_dump($args);
//        exit;
        foreach ($args as $key => $value) {
            $args[$key] = trim($value);
    ...
    //修改数据
    public static function updatePtwCondition($args) {
//        var_dump($args);
//        exit;
        foreach ($args as $key => $value) {
            $args[$key] = trim($value);
        ...
        $model = PtwCondition::model()->findByPk($args['condition_id']);
    ...
    //删除数据
    public static function deletePtwCondition($condition_id) {
        ...
        $model = PtwCondition::model()->findByPk($condition_id);
        ...
        $sql = 'UPDATE ptw_condition_list SET status=:status  WHERE condition_id=:condition_id';
    ...
    public static function conditionList() {
        $sql = "select * from ptw_condition_list where status = '00'";
        $command = Yii::app()->db->createCommand($sql);
    ...
    public static function detailList($type_id){
        ...
        $sql = "SELECT * FROM ptw_condition_list WHERE  type_id = '".$type_id."' and status = '00' ";//var_dump($sql);
        $command = Yii::app()->db->createCommand($sql);
        $rows = $command->queryAll();
        if(!empty($rows)){
            foreach($rows as $i => $j){
