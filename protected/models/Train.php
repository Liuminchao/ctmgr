...
class Train extends CActiveRecord {
    ...
    public static function queryList($page, $pageSize, $args = array()) {
        ...
        if($args['user_id'] != ''){
            if($args['deal_type'] != -1) {
                $sql = "SELECT b.training_id FROM bac_check_apply_detail_train a,train_apply_basic b WHERE a.deal_user_id = '".$args['user_id']."' and a.app_id = 'TRAIN' and a.apply_id = b.training_id and b.program_id = '".$args['program_id']."' and b.module_type='".$args['module_type']."'and b.type_id ='".$args['type_id']."' and a.deal_type = '" . $args['deal_type'] . "'";
    ...
    //人员按权限和成员进行统计
    public static function findBySummary($user_id,$program_id,$module_type,$date){

        $sql_1 = "SELECT a.type_id, b.deal_type, count(distinct b.apply_id) as cnt
                  FROM train_apply_basic a inner join bac_check_apply_detail_train b
                  ...
                  group by a.type_id";
                // UNION
        $sql_2="SELECT c.type_id, 'MEMBER' as deal_type, count(distinct c.training_id) as cnt
        ...
        $status_css = CheckApplyDetailTrain::statusText();//PTW执行类型
    ...
    //统计总次数
    public static function cntBySummary($user_id,$program_id,$module_type,$date){
...
//        }else{
            if($module_type == 1) {
                $filepath = Yii::app()->params['upload_report_path'].'/'.$year.'/'.$month.'/'.$program_id.'/train/'.$meeting->add_conid.'/TRAIN' . $id . $time .'.pdf';
                $full_dir = Yii::app()->params['upload_report_path'].'/'.$year.'/'.$month.'/'.$program_id.'/train/'.$meeting->add_conid;
                ...
                $filepath = Yii::app()->params['upload_report_path'].'/'.$year.'/'.$month.'/'.$program_id.'/meeting/'.$meeting->add_conid.'/MEETING' . $id . $time .'.pdf';
                $full_dir = Yii::app()->params['upload_report_path'].'/'.$year.'/'.$month.'/'.$program_id.'/meeting/'.$meeting->add_conid;
...
//        }
        ...
        if (!empty($progress_list)){
            foreach ($progress_list as $key => $row) {
                if($row['pic'] != '' || $row['pic'] != '-1' || $row['pic'] != 'nil') {
                    $pic = explode('|', $row['pic']);
                    // var_dump($pic);
                    // exit;
                    if($y2 > 266){
                    ...
                    foreach ($pic as $key => $content) {
                        ...
                        if($content != '' && $content != 'nil' && $content != '-1') {
                            if(file_exists($content)) {
                                ...
                                // }
                                $img = $content;
                                // $content = 'https://shell.cmstech.sg/test/ctmgr/thumb_Q-checklist_1585627834152_1.jpg';
                                $ratio_width = 55;
                                //超过固定宽度换行
                                if($toatl_width > 190){
                                    $toatl_width = 0;
