 ...
 * @author LiuXiaoyuan
 */
class TypeCondition extends CActiveRecord
{
	/**
	 ...
	 * @return TypeCondition the static model class
	 */
    ...
    public static function queryList($page, $pageSize, $args = array()) {
        ...
        $total_num = TypeCondition::model()->count($condition, $params); //总记录数
        ...
        $rows = TypeCondition::model()->findAll($criteria);
    ...
    //插入数据
    public static function insertTypeCondition($args) {
        ...
        $exist_data = TypeCondition::model()->count('type_id=:type_id', array('type_id' => $args['type_id']));
        if ($exist_data != 0) {
            $r['msg'] = '该记录已存在。';
            $r['status'] = -1;
            $r['refresh'] = false;
        ...
        try {
            $model = new TypeCondition('create');
    ...
    //修改数据
    public static function updateTypeCondition($args) {
        ...
        $model = TypeCondition::model()->findByPk($args['type_id']);
    ...
    //删除数据
    public static function deleteTypeCondition($id) {
        ...
        $model = TypeCondition::model()->findByPk($id);
