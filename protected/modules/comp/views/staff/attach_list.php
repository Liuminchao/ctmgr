...
//    exit;
    ...
    foreach ($rows as $i => $row) {
        ...
        $delete_link = "<a href='javascript:void(0)' onclick='itemDelete(\"{$row['aptitude_id']}\",\"{$row['user_id']}\")'><i class=\"fa fa-fw fa-times\"></i>" . Yii::t('electronic_contract', 'delete') . "</a>";
        ...
        $edit_link = "<a href='javascript:void(0)' onclick='itemEdit(\"{$row['aptitude_id']}\",\"{$row['user_id']}\")'><i class=\"fa fa-fw fa-edit\"></i>".Yii::t('common', 'edit')."</a>";//编辑
        if($type == 'mc'){
...
//            $t->echo_td("<a href='javascript:void(0)' onclick='itemPreview(\"{$row['doc_path']}\",\"{$row['doc_id']}\")'>" . $row['doc_name'] . "</a>");
            $t->echo_td("<a href='index.php?r=comp/staff/preview&aptitude_photo={$row['aptitude_photo']}' target='_blank'><i class=\"fa fa-fw fa-eye\"></i>" . $row['aptitude_content']. "</a>",'center');
        }else{
            $t->echo_td("<a href='javascript:void(0)' onclick='window.open(\"{$row['aptitude_photo']}\",\"_blank\")'><i class=\"fa fa-fw fa-eye\"></i>" . $row['aptitude_content']. "</a>",'center');
        }
