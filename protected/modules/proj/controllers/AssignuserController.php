...
class AssignuserController extends BaseController {
     ...
     * 分包权限查看表头
     */
    private function genSubAuthorityGrid($program_id,$contractor_id) {
        $t = new DataGrid($this->gridId);
        $t->url = 'index.php?r=proj/assignuser/subauthoritygrid&program_id='.$program_id.'&contractor_id='.$contractor_id;
        $t->updateDom = 'datagrid';
     ...
     * 分包权限查询
     */
    public function actionSubAuthorityGrid($program_id,$contractor_id) {
        $page = $_GET['page'] == '' ? 0 : $_GET['page']; //当前页码
        $_GET['page'] = $_GET['page'] + 1;
        $args = $_GET['q']; //查询条件
        $args['program_id'] = $program_id;
        ...
        $t = $this->genSubAuthorityGrid($program_id,$contractor_id);
        $this->saveUrl();
        $this->saveSubEpssUrl();
    ...
    public function actionSubAuthorityList() {
        ...
        $this->render('subauthoritylist', array('program_id' => $program_id,'contractor_id'=>$contractor_id,'contractor_name' => $contractor_name));
    }
    ...
    public function actionGetptwSource() {
        $program_id = $_REQUEST['program_id'];
        $program_model = Program::model()->findByPk($program_id);
        $root_proid = $program_model->root_proid;
        $root_model = Program::model()->findByPk($root_proid);
        $contractor_id = $program_model->contractor_id;
        $contractor_model = Contractor::model()->findByPk($contractor_id);
        $contractor_type = $contractor_model->contractor_type;
        if($root_model->params != '0'){
            $params = json_decode($root_model->params,true);
            if(!array_key_exists('ptw_mode',$params)){
                $params['ptw_mode'] = 'A';
            }
        }else{
    ...
    public  function actionEpss(){
        ...
        if (isset($_POST['ProgramUser'])) {
            ...
            $msg = $r;
            if ($r['refresh'] == true) {
                $model->_attributes = $_POST['ProgramUser'];
            }else{
                $model->_attributes = ProgramUser::model()->find('user_id=:user_id and program_id=:program_id', array(':user_id' => $user_id,':program_id' => $program_id));
            }
...
//============================================================+
    ...
    //导出员工信息表
    public static function actionEpssExport(){
        $args = $_GET['q'];
        $args['contractor_id'] = Yii::app()->user->getState('contractor_id');
        $rs = ProgramUser::staffinfo($args);
        ...
        $objectPHPExcel = $objReader->load("./template/excel/Pro Epss.xlsx");
        ...
        foreach ($rs as $k => $v) {
            // static $n = 1;
            $objectPHPExcel->getActiveSheet()->setCellValue(A . ($k + 3),$v['user_id']);
            $objectPHPExcel->getActiveSheet()->setCellValue(B . ($k + 3),$v['user_name']);
            $objectPHPExcel->getActiveSheet()->setCellValue(C . ($k + 3),$v['user_phone']);
            $objectPHPExcel->getActiveSheet()->setCellValue(D . ($k + 3),$v['work_no']);
            // $n++;
        }
        //下载输出
        ob_end_clean();
        //ob_start();
        header('Content-Type : application/vnd.ms-excel');
        header('Content-Disposition:attachment;filename="'.'Employee Epss information table-'.date("d M Y").'.xls"');
        $objWriter= PHPExcel_IOFactory::createWriter($objectPHPExcel,'Excel5');
        $objWriter->save('php://output');
    }
    ...
    public function actionQr() {
        $user_id = $_REQUEST['user_id'];
        $program_id = $_REQUEST['program_id'];
        $this->renderPartial('print_qrcode',array('user_id'=>$user_id,'program_id'=>$program_id));
    }
     ...
     * 保存查询链接
     */
    private function saveMcEpssUrl() {

        //$a = Yii::app()->session['list_url'];
        $a['epss/list'] = str_replace("r=proj/assignuser/authoritygrid", "r=proj/assignuser/authoritylist", $_SERVER["QUERY_STRING"]);
        Yii::app()->session['list_url'] = $a;
    }
     ...
     * 保存查询链接
     */
    private function saveSubEpssUrl() {

        //$a = Yii::app()->session['list_url'];
        $a['epss/list'] = str_replace("r=proj/assignuser/subauthoritygrid", "r=proj/assignuser/subauthoritylist", $_SERVER["QUERY_STRING"]);
        Yii::app()->session['list_url'] = $a;
    }
    ...
    public function actionCreateQrPdf() {
        ...
        if(count($pagedata)>0){
            ...
            $file_path = ProgramUser::downloadQrPdf($result,$program_id);
            $filepath_cnt = $redis->lPush('user-list', $file_path);
            $redis->set('user_cnt', $filepath_cnt);
    ...
    public function actionDownloadQrZip() {
        $filename = ProgramUser::createPbuZip();
        if (file_exists($filename) == false) {
            header("Content-type:text/html;charset=utf-8");
            echo "<script>alert('".Yii::t('common','Document not found')."');</script>";
            return;
        ...
        $redis->connect('127.0.0.1', 6379);
        $redis->select(7);
        $redis->delete('user-list');
        $redis->delete('user_cnt');
        $redis->close();
    ...
    public function actionSyncUser() {
        $program_id = $_REQUEST['program_id'];
        $rs = ProgramUser::UserSync($program_id);
        echo json_encode($rs);
    }
    ...
    public function actionSyncWorkforce() {
        ...
        $url = 'https://w.beehives.sg/admin/web/index.php?r=manpower/member&app_id=APPP001&bes_company_id='.$bes_company_id."&bes_project_id=".$bes_project_id."&sc_program_id=".$program_id;
