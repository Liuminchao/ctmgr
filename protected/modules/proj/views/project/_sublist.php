...
if (is_array($rows)) {
    ...
    foreach ($rows as $i => $row) {
        ...
        $team_link = "<a href='javascript:void(0)' onclick='itemTeam(\"{$row['program_id']}\",\"{$row['contractor_id']}\",\"{$row['program_name']}\")'><i class=\"fa fa-fw fa-user\"></i>" . Yii::t('proj_project_user', 'project_team') . "</a>&nbsp;";
        $device_link = "<a href='javascript:void(0)' onclick='itemDevice(\"{$row['program_id']}\",\"{$row['program_name']}\")'><i class=\"fa fa-fw fa-gear\"></i>". Yii::t('proj_project_user', 'device') . "</a>&nbsp;";
        ...
        $stop_link = "<a href='javascript:void(0)' onclick='itemStop(\"{$row['program_id']}\",\"$pro_name\",\"$contractor_name\")'><i class=\"fa fa-fw fa-check\"></i>" . Yii::t('proj_project', 'STATUS_STOP') . "</a>&nbsp;";
        $workforce_link = "<a href='javascript:void(0)' onclick='itemWorkforce(\"{$row['program_id']}\")'><i class=\"fa fa-fw fa-users\"></i>Sync (Manpower)</a>&nbsp;";
        ...
        if ($row['status'] == Program::STATUS_NORMAL) {
            if($row['add_conid'] == Yii::app()->user->contractor_id){
                if($value == '0'){
                    ...
                    $link = "<table><tr><td style='white-space: nowrap' align='left'>$team_link</td><td style='white-space: nowrap' align='left'>$edit_link</td></tr><tr><td style='white-space: nowrap' align='left'>$del_link</td><td style='white-space: nowrap' align='left'>$stop_link</td></tr><tr><td style='white-space: nowrap' align='left'>$device_link</td><td style='white-space: nowrap' align='left'>$workforce_link</td></tr></table>";
                }
            }
        }
        ...
        if($row['sub_type_id']){
            $t->echo_td($type_list[$row['sub_type_id']],'center');
        }else{
            $t->echo_td($row['sub_type_id'],'center');
        }
        //$t->echo_td($row['contractor_id'],'center');
        if($in_staff_cnt > 0){
            $t->echo_td("<a class='omit'  href='javascript:void(0)' onclick='itemSubStaff(\"{$row['contractor_id']}\",\"{$row['program_id']}\",\"in\")'>".$in_staff_cnt."</a>",'center'); //Contractor
        }else{
            $t->echo_td($in_staff_cnt,'center');
        }
        if($out_staff_cnt > 0){
            $t->echo_td("<a class='omit'  href='javascript:void(0)' onclick='itemSubStaff(\"{$row['contractor_id']}\",\"{$row['program_id']}\",\"out\")'>".$out_staff_cnt."</a>",'center'); //Contractor
        }else{
